...
function isbnConverter(isbn) {
    let remove_last_character = isbn.split("-").slice(0, -1);
    remove_last_character.unshift("978")
    let twelve_digits = remove_last_character.join("").split("")
    let sum = 0
    for (let i = 0; i < twelve_digits.length; i++) {
        if (i % 2) {
            sum += parseInt(twelve_digits[i]) * 3
        } else {
            sum += parseInt(twelve_digits[i]) * 1
        }
    }
    let check_digit = 10 - parseInt(sum) % 10 === 10 ? 0 : 10 - parseInt(sum) % 10;
    remove_last_character.push(check_digit.toString())
    return remove_last_character.join("-");
}